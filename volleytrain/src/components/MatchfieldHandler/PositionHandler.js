...
export function PostPutHandler(MatchfieldPlayerBOs, posPlayers, dimensions) {
  var posTmp = [];
  // get all objects that are on the field -> these will be saved
  ...
  posTmp.forEach((player) => {
    if (MatchfieldPlayerBOs.some(player.id)) {
      console.log(player);
    }
  });
  return posTmp;
